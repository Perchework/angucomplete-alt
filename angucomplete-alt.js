...
angular.module('angucomplete-alt', [])
    .directive('angucompleteAlt', ['$q', '$parse', '$http', '$sce', '$timeout', '$templateCache', '$filter', function ($q, $parse, $http, $sce, $timeout, $templateCache, $filter) {
        // keyboard events
        var KEY_DW = 40;
        var KEY_RT = 39;
        var KEY_UP = 38;
        ...
        $templateCache.put(TEMPLATE_URL,
            ...
            '      </div>' +
            '      <div class="angucomplete-title" ng-if="matchClass" class="angucomplete-title" ng-bind-html="result.title"></div>' +
            '      <div class="angucomplete-title" ng-if="!matchClass"><b>{{ result.title }}</b></div>' +
            ...
            link: function (scope, elem, attrs, ctrl) {
                ...
                function extractValue(obj, key) {
                    ...
                    if (angular.isDate(result)) {
                        result = $filter('date')(result, 'dd/MM/yyyy');
                        result = '<span class="angucomplete-date">' + result + '</span>';
                    }
                    return result;
                }

                function findMatchString(target, str) {
                    var result = target, matches, re;
                    if (!target) {
                        return;
                    }
